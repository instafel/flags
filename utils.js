    ...
    if (groupedArray[category] != undefined) {
      ...
      for (const item of sortedItems) {
        ...
        fs.copyFileSync(
          ...
          path.join(pathList, "mds", item.frontmatter.id + ".md")
        );
      }
      fs.writeFileSync(
        path.join(pathList, "dir-list", category + ".json"),
...
function updateLastEditFields(arrayContent) {
  ...
  if (changed.length === 0) {
    console.log("No any markdown file updated, every md is up-to-date");
  } else {
    changed.forEach((file) => {
      if (fs.existsSync(file)) {
        ...
        console.log(
          `✅ ${file}'s last_edit updated to ${updatedData.last_edit}`
        );
      }
    });
  }
...
function getChangedMarkdownFiles() {
  try {
    ...
    console.error(
      "❌ An error occured while tracking commit files:",
      err.message
    );
    return [];
  }
...
function organizeIDs(arrayContent) {
  for (const item of arrayContent) {
    if (item.frontmatter.id == null) {
    }
  }

  const idCount = {};
  for (const item of arrayContent) {
    idCount[item.frontmatter.id] = (idCount[item.frontmatter.id] || 0) + 1;
  }
  ...
  var dumpLock = false;
  for (const dumpId of duplicates) {
    if (!isNaN(dumpId)) {
      console.log(
        "ID " +
          dumpId +
          " is duplicated, please change it and try to re-generate."
      );
      dumpLock = true;
    }
  }
  ...
  const idList = arrayContent.map((item) => item.frontmatter.id);
  ...
  arrayContent.forEach((item) => {
    if (item.frontmatter.id == null) {
      ...
      console.log(
        `Content ${item.frontmatter.title}'s ID is setted to ${newID}`
      );

      updateFileID(item, newID);
    }
  });
}

function updateFileID(item, newID) {
  const fPath = path.join(pathContent, item.filename);
  const fContent = fs.readFileSync(fPath, "utf-8");
  const lines = fContent.split("\n");

  const updatedLines = lines.map((line) => {
    if (line.includes("id: null")) {
      return line.replace("id: null", `id: ${newID}`);
    }
    return line;
  });

  fs.writeFileSync(fPath, updatedLines.join("\n"), "utf-8");
}

function getMarkdownDatas() {
  const markdownFiles = fs
    .readdirSync(pathContent)
  ...
  for (const file of markdownFiles) {
    ...
    if (missingFields.length == 0) {
      if (categories.includes(frontmatter.category)) {
        ...
        console.log(
          `✅ Flag ${frontmatter.title} (${
            frontmatter.id != null ? frontmatter.id : "ID will be reseved later"
          }) pass`
        );
      } else {
        console.log(
          `❌ File ${file} has unknown category, please check it and re-generate before publish`
        );
      }
    } else {
      console.log(
        `File ${file} has missing data fields, please check it and re-generate before publish`
      );
    }
  }
...
function cleanListFolder() {
  if (fs.existsSync(pathList)) {
    ...
    fs.mkdirSync(path.join(pathList, "dir-list"));
    fs.mkdirSync(path.join(pathList, "mds"));
  } else {
    fs.mkdirSync(pathList);
    fs.mkdirSync(path.join(pathList, "dir-list"));
    fs.mkdirSync(path.join(pathList, "mds"));
